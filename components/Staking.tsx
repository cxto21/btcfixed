  ...
  if (status === 'pending') {
    return (
      <div className="flex items-center gap-3 p-4 border-2 border-black dark:border-white bg-yellow-50 dark:bg-yellow-900/20">
        ...
        <span className="text-xs font-bold tracking-widest uppercase dark:text-white">Transacción en progreso…</span>
      </div>
    );
  }
  if (status === 'success' && txHash) {
    return (
      <div className="flex items-center gap-3 p-4 border-2 border-black dark:border-white bg-green-50 dark:bg-green-900/20">
        ...
        <span className="text-xs font-bold tracking-widest uppercase dark:text-white flex-1">¡Transacción exitosa!</span>
  ...
  if (status === 'error') {
    return (
      <div className="flex items-center gap-3 p-4 border-2 border-black dark:border-white bg-red-50 dark:bg-red-900/20">
        ...
        <span className="text-xs font-bold tracking-widest uppercase dark:text-white">Transacción fallida. Intenta de nuevo.</span>
      </div>
    );
  }
  return null;
...
function ValidatorSelector({ selected, onChange }: { selected: ValidatorKey; onChange: (k: ValidatorKey) => void }) {
  return (
    <div className="space-y-2">
      <p className="text-[10px] font-black tracking-widest opacity-40 dark:opacity-60 uppercase dark:text-white">Validador</p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-[2px] bg-black dark:bg-white border-2 border-black dark:border-white overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.1)]">
        <div className="bg-white dark:bg-black p-6 flex justify-between items-center">
          <div>
            <p className="text-[10px] font-black tracking-widest mb-1 opacity-30 dark:opacity-70 uppercase dark:text-white">APY Efectivo</p>
          ...
          <div className="text-right">
            <p className="text-[10px] font-black tracking-widest mb-1 opacity-30 dark:opacity-70 uppercase dark:text-white">Comisión</p>
            <p className="text-lg font-bold dark:text-white">{commission !== null ? `${commission}%` : '—'}</p>
          </div>
        </div>
        <div className="bg-white dark:bg-black p-6">
          <p className="text-[10px] font-black tracking-widest mb-1 opacity-30 dark:opacity-70 uppercase dark:text-white">Protocolo</p>
      ...
      {position && (
        <div className="border-2 border-black dark:border-white bg-white dark:bg-black p-5 space-y-4">
          <p className="text-[10px] font-black tracking-widest opacity-40 dark:opacity-60 uppercase dark:text-white">Tu Posición</p>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-[9px] opacity-40 dark:opacity-60 uppercase tracking-widest dark:text-white mb-1">En staking</p>
          ...
          {unpoolingDisplay && (
            <div className="flex items-start gap-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-400">
              ...
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest dark:text-white">{unpoolingDisplay} STRK en salida</p>
                {unpoolTime && (
                  <p className="text-[9px] opacity-60 mt-0.5 dark:text-white">Disponible: {unpoolTime.toLocaleDateString('es', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                )}
      ...
      <div className="flex border-2 border-black dark:border-white bg-white dark:bg-black p-1">
        {(['stake', 'unstake', 'claim'] as const).map((a) => (
          ...
          >
            {a === 'stake' ? 'Hacer Stake' : a === 'unstake' ? 'Retirar' : 'Cobrar'}
          </button>
        ))}
      </div>
        ...
        <div className="p-8 border-2 border-black dark:border-white space-y-6 bg-white dark:bg-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.05)]">
          <div className="flex border-b-2 border-black dark:border-white pb-6">
            <div className="flex-1">
              ...
              <p className="text-[10px] font-bold opacity-30 dark:opacity-70 uppercase tracking-[0.2em] mt-2 dark:text-white">
                Cantidad STRK · Disponible: {strkAvailable}
              </p>
            </div>
          ...
          <div className="flex justify-between items-center text-xs font-bold dark:text-white">
            <div className="flex items-center gap-2">
              ...
              <span className="tracking-widest uppercase">Retorno anual estimado</span>
            </div>
          ...
          <button onClick={handleStake} disabled={isPending || !inputValue || parseFloat(inputValue) <= 0 || !address}
            ...
            {isPending ? <><Loader2 size={16} className="animate-spin" />Confirmando…</> : 'Hacer Stake'}
          </button>
        </div>
      )}
        ...
        <div className="p-8 border-2 border-black dark:border-white space-y-6 bg-white dark:bg-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.05)]">
          <div className="flex border-b-2 border-black dark:border-white pb-6">
            <div className="flex-1">
              <input type="number" value={unstakeInput} onChange={(e) => setUnstakeInput(e.target.value)} placeholder="0.00" min="0"
                className="text-4xl font-bold bg-transparent outline-none w-full tracking-tighter dark:text-white" />
              <p className="text-[10px] font-bold opacity-30 dark:opacity-70 uppercase tracking-[0.2em] mt-2 dark:text-white">
                STRK a retirar · En staking: {stakedDisplay}
              </p>
            </div>
            {position && (
          ...
          <div className="flex items-start gap-2 p-3 border border-yellow-400 bg-yellow-50 dark:bg-yellow-900/20">
            ...
            <p className="text-[10px] font-medium dark:text-white leading-relaxed">
              El retiro inicia un período de salida. Los fondos dejan de generar rewards y estarán disponibles al finalizar el lockup del protocolo.
            </p>
          </div>
          <button onClick={handleExitIntent} disabled={isPending || !unstakeInput || parseFloat(unstakeInput) <= 0 || !position}
            className="w-full h-14 bg-black text-white dark:bg-white dark:text-black font-bold text-sm tracking-[0.2em] uppercase disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            {isPending ? <><Loader2 size={16} className="animate-spin" />Confirmando…</> : <><ArrowDownLeft size={16} />Iniciar Retiro</>}
          </button>
        </div>
      )}
      ...
      {activeAction === 'claim' && (
        <div className="p-8 border-2 border-black dark:border-white space-y-6 bg-white dark:bg-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.05)]">
          <div className="text-center space-y-3 py-4">
            ...
            <p className="text-[10px] font-black tracking-widest uppercase dark:text-white opacity-40">Rewards acumulados</p>
          ...
          <button onClick={handleClaim} disabled={isPending || !hasRewards}
            className="w-full h-14 bg-black text-white dark:bg-white dark:text-black font-bold text-sm tracking-[0.2em] uppercase disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            {isPending ? <><Loader2 size={16} className="animate-spin" />Confirmando…</> : <><Gift size={16} />Cobrar Rewards</>}
          </button>
        </div>
      )}
      ...
      <div className="flex items-start gap-4 p-5 border-2 border-black dark:border-white bg-muted/50 dark:bg-zinc-900/50">
        ...
        <div>
          <p className="text-xs font-black uppercase tracking-widest mb-1 dark:text-white">Staking Nativo</p>
          <p className="text-[11px] text-black/60 dark:text-white/80 leading-relaxed font-medium">
            Delegas tus STRK a un validador del protocolo Starknet. Tus fondos <strong>nunca salen de tu billetera</strong> — solo delegas el poder de staking.
          </p>
        </div>
      </div>
    </div>
...
function ComingSoon({ label }: { label: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 space-y-4 opacity-40">
      ...
      <p className="text-xs font-black uppercase tracking-widest dark:text-white">{label} — Próximamente</p>
    </div>
  );
}
